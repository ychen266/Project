...
class Workstation(myNode):  # Define a workstation node type
    ...
    equivalence = ["workstation", "assembly position","worked_time","Repair_Time"]

    def __init__(self, workstation,worked_time,repair_time):
        self.nodeProperties = {}
        self.nodeProperties["WorkstationID"] = workstation
        self.nodeProperties["Worked_Time"] = worked_time
        self.nodeProperties["Repair_Time"] = repair_time
        super().__init__(self.nodeType, **self.nodeProperties)
...
class Entities():  # Define a class whcih can store lists of all node types
    ...
    def addWorkstation(self, workstation,worked_time,repair_time):
        # add workstation as a Node object to the workstation list
        workstationNode = Workstation(workstation,worked_time,repair_time)
        ...
        if node['WorkstationID'] == workstation:
             node['Worked_Time'] += 1
        if node['Worked_Time']==4:
            print(f"Workstation {node['WorkstationID']} needs repair.")
            print(node['WorkstationID'])
            print(node['Worked_Time'])
    ...
    def extractWorkstationFromFile(self, datafile, sheetname='Workstation List'):
        ...
        worked_time = workstationinfo["Worked_Time"].tolist()
        repair_time = workstationinfo["Repair_Time"].tolist()
        for i in range(len(workstation)):
            self.addWorkstation(workstation[i], worked_time[i],repair_time[i])
