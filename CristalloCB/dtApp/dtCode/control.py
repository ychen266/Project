    ...
    if request.method=='POST':
        ...
        if cntrtype=="passive":
            ...
            kinenergy = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
                ...
                # Save the data in input file data file with the time/date

            num_ = len(os.listdir('./dtApp/dtData/knowledgeGraph/'))
            count_ = 0
            if num_> 0:
                count_ = int((num_) /3)
            np.save('dtApp/dtData/knowledgeGraph/' +str(count_+1)+  '_mb' , mb)
            np.save('dtApp/dtData/knowledgeGraph/'+str(count_+1)+'_x',freq_TMD)    
            np.save('dtApp/dtData/knowledgeGraph/'+str(count_+1)+'_y',T_TMD)
